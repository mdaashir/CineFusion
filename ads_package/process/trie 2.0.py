...
class Trie:
    ...
    def formTrie(
        self, keys
    ):  # Function to access the keys from list and insert it into trie data structure.
        for key in keys:
            self.insert(key)
...
# Read movie names from the CSV file and append them to the 'keys' list
try:
    with open(csv_file, newline="", encoding="utf-8") as file:
        ...
        for row in reader:
            ...
            if movie_title:
                keys.append(movie_title.lower())  # Append movie name to the 'keys' list
...
def view():
    ...
    listBox2 = tk.Listbox(
        root, listvariable=History, selectmode=tk.EXTENDED
    )  # To Display the Browser History in the listbox
...
name_entry = tk.Entry(
    root, textvariable=word, font=("calibre", 10, "normal")
)  # The entrybox for the user in which he types the word he
name_entry.grid(row=0, column=1)  # wants to search for

vari = tk.Variable(value=keys)
listbox = tk.Listbox(
    root, listvariable=vari, height=6, selectmode=tk.EXTENDED
)  # Listbox to display all words searched from Trie.
listbox.grid(row=2, column=1)

sub_btn = tk.Button(
    root, text="Search", command=submit
)  # Button when clicked show a pop up box that search
sub_btn.grid(
    row=10, column=1
)  # was successful and append to history when the word is searched

bt = tk.Button(
    root, text="Browser History", command=view
)  # Button to view the Browser history
...
listbox.bind(
    "<<ListboxSelect>>", fillout
)  # To execute a function when the selected items change,
